...
> **2026-09-18 — NEXT SESSION STARTS HERE: the 6.0 port goes LIVE on the sandbox (the user, 2026-09-18: *"prep for a new session, i presume i am ready for testing"*).** Not yet testable by hand: the sandbox still runs the PRE-port module (`deploy-house-module.mjs fvtt-mod-fxstudio --local --check` says 12 files differ — the reader, the engine, module.json, the six Stock files), and prod is 5.3.3 so nothing 6.0 goes there. The runbook, in order, once the user says the sandbox is theirs to give (another agent was in it on 2026-09-15; at 2026-09-18 it was ACTIVE with one user connected): (1) `disconnect-bridge` on foundry-local5e if it is connected; (2) `node ../fvtt-mcp-molten5e/scripts/local-foundry.mjs stop`; (3) `node ../fvtt-mcp-molten5e/scripts/deploy-house-module.mjs fvtt-mod-fxstudio --local` — module.json changed (the pin), so a restart is required, and `recipes/` must travel (tools/README); then `--check` must say byte-identical; (4) `local-foundry.mjs start`; (5) `node tools/smoke-boot.mjs` (stock must read 1014, house 4), then `smoke-fx`, `smoke-replay`, `smoke-author`, `smoke-screens` — fix what they name, one green pass, and expect the replay's template sections (§6–7: a Region placed the 6.0 way) and the faked usage cards (`system` data on `ChatMessage.create`) to be where 6.0 teaches something; (6) THEN the user tests by hand on the sandbox at localhost:30000 — cards, areas, effects, Battle Flow's hold beside it; (7) a release on the user's word (v0.6.0: the pin, the port, Stock 1014), NOT to prod until prod is upgraded to Foundry 14.367+ and dnd5e 6.0.x. Everything is committed and pushed (eeb5403 at the top; the port is 1103efc → 6656ece, the roadkill cut eeb5403); the nine offline checks are green; ASSESSMENT-6.0.md is the record and §7 holds the rulings, with C (actor identifiers) and D (turn cards) still open for phase 4.
