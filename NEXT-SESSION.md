...
> **2026-09-19 — THE USER'S TESTING HAS STARTED; the first two things named are fixed and on the sandbox (not released).** *"when i cast persistent AOE effects, like fog cloud and web, the template region stays on the ground. it should fade out and leave the vfx showing. also, when gren casted web, the web vfx itself didnt appear at all."* The first was the module (nothing ever hid the Region dnd5e 6.0 places): a fill that persists with its template now hides the Region from the table in dnd5e's pre-create hook, LAYER visibility, the GM keeps it on the Regions layer — DESIGN §24 has the rule and the cost (no fade; the area is bare until the picture lands; a player caster's one Stock `clearTemplate` cannot delete its Region, noted not fixed). The second was the JSON: Web was AA's range row (strands to targets, the web on them), re-authored as a fill at the template for both `spell:web` and `feature:web`. Nine checks green (check-reader 124/124); deployed `--local` byte-identical, no module.json change, a reload is enough. **Then, the same day, the word — *"commit/push/release"* — and v0.6.0 is RELEASED** (tag pushed, GitHub release with the zip and the bare module.json, notes hand-written in dist/); the sandbox carries the 0.6.0 manifest byte-identical but its process vends 0.5.0 until a restart, which the user's session blocks — harmless. **Then, 2026-09-21: prod upgraded to dnd5e 6.0 and v0.6.0 DEPLOYED TO PROD** (the user: *"prod is now updated, please deploy to prod"*). The `--check` first showed prod's house.json at a hash the repo never held; the user ruled *"prod is canonical"*; prod's copy was fetched over WebDAV and held two House overrides saved there 2026-09-16 — greatsword (the Fire Touched Ember Sword, fire-orange) and maul (the Maul of Momentum, white), both keyed to the BASE weapon so every greatsword and maul in the world plays them — pulled into the repo, checks green, then the deploy: byte-identical, 50 files. Prod's six Stock files had been exactly the 2026-09-13 deploy. Prod vends 0.6.0 after its next process restart; scripts and recipes are live on reload. The MCP repo is `../fvtt-mcp-dnd5e` now (was molten5e) and prod is `FOUNDRY_HOST=molten`. **The user keeps testing, on prod and the sandbox.**
