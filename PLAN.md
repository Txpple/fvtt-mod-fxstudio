...
in §6 met by a DOM-driven suite (`tools/smoke-screens.mjs`, 84 of 84 after the vocabulary pass and the FX sheet of 2026-09-06, DESIGN §8) and left for the user to do
...
**Backlog (the user, 2026-09-06: "yes battleflow is backlog").** Phase 4 is built on dnd5e and
Foundry core alone: the attack message carries each target's hit or miss, the save message each
target's pass or fail, dnd5e's damage hook the damage applied by type, conditions arrive as
active effects, combat turns and token moves are core hooks. The moments that exist only in
Battle Flow's rules — Riposte, a held Shield, a maneuver die spent, a shield paid, a fold,
emanation members gained or lost — can only ever arrive as hooks from it, and that commission
(public hooks with plain payloads at its resolve points; no dependency, no setting, no flag
