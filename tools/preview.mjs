// Play an FX on the sandbox fixture so a person or an assistant can see it before proposing it.
// Live: builds the fixture (a caster, a target six squares away), plays the FX once through the
// API's preview (never saved), reports what played from the ledger, tears the fixture down.
//
//   node tools/preview.mjs fire-bolt                 # an FX by id, from the recipes as deployed
//   node tools/preview.mjs my-fx.json              # an FX file (one FX, a list, or {fx: [...]})
//   node tools/preview.mjs my-fx.json --template   # with a circle placed on the target (an area fx)
//   node tools/preview.mjs … --watch 6000            # keep the fixture up that long after playing, to fx
import { existsSync } from 'node:fs';
import { connectSandbox } from './lib/foundry.mjs';
import { fixtureDown, fixtureUp } from './lib/suite.mjs';
import { readFxFile } from './lib/recipes.mjs';

const args = process.argv.slice(2);
const what = args.find((a) => !a.startsWith('--'));
if (!what) { console.error('usage: node tools/preview.mjs <fx id | fx file> [--template] [--watch ms]'); process.exit(2); }
const watch = args.includes('--watch') ? Number(args[args.indexOf('--watch') + 1]) || 6000 : 2500;
const withTemplate = args.includes('--template');
const fx = existsSync(what) ? readFxFile(what) : null;
...
try {
  ...
  const out = await f.evaluate(async ({ id, fx, withTemplate, casterTokenId, targetTokenId, watch }) => {
    ...
    for (const fx of list) {
      const problems = api.fx.validate(fx);
      if (problems.length) { results.push({ id: fx.id, problems }); continue; }
      const r = await api.preview(fx, { source: caster, targets: [target], place: region, destination: { x: 850, y: 850 } });
      results.push({ id: fx.id, sentence: r.sentence, entry: r.entry && { played: r.entry.played, files: r.entry.files, sounds: r.entry.sounds, missing: r.entry.missing, why: r.entry.why }, problems: r.problems ?? [] });
      await sleep(watch);
    }
    if (region && canvas.scene.regions.get(region.id)) await canvas.scene.deleteEmbeddedDocuments('Region', [region.id]);
    return { results };
  }, { id: what, fx, withTemplate, casterTokenId: fixture.casterTokenId, targetTokenId: fixture.targetTokenId, watch });
