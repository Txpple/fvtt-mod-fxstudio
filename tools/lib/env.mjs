...
// classic-level (the LevelDB reader) is this repo's own declared dependency (package.json) — it
// used to be reached through the MCP repo's node_modules, which its rename broke.
const require = createRequire(import.meta.url);
export function classicLevel() {
  try {
    return require('classic-level').ClassicLevel;
  } catch (e) {
    throw new Error(`classic-level is not installed; run npm install in ${REPO} (${e.message})`);
  }
}
