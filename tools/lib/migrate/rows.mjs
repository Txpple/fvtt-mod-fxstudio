...
export function rowToFx(row, nativiser, { id, keys, on }) {
  ...
  switch (primary?.preset) {
    ...
    case 'teleport': {
      ...
      const o = d.options;
      const move = { shape: 'move', range: o.range, speed: o.speed, jump: !!o.teleport, after: o.delayMove, ...(o.checkCollision ? {} : { seen: false }) }; // AA's Check Collision was the user's own "walls matter" per fx: it is now "a space you can see" (the default); off means the spot need not be seen
    ...
    case 'projectile-to-template': {
      ...
      if (d.preExplosion?.enable && (d.preExplosion.aa || d.preExplosion.file)) { const o = d.preExplosion.options; scenes.push(trim({ shape: 'mark', at: 'template', asset: assetOf(d.preExplosion, 'pre-explosion'), sound: preSound, size: { fit: 'object', scale: o.scale }, ...repeatOf(o), rate: o.playbackRate, wait: o.wait === 0 ? true : o.wait, ...elevationOf(o, 'both'), fadeIn: 0, fadeOut: 0, zIndex: 0 })); }
      else if (preSound) scenes.push({ shape: 'sound', ...preSound });
      { const o = d.explosion.options; scenes.push(trim({ shape: 'mark', at: 'template', asset: assetOf(d.explosion, 'explosion'), sound: soundOf(d.explosion.sound), size: { fit: 'object', scale: o.scale }, ...repeatOf(o), zIndex: 5, rate: o.playbackRate, wait: -750 + o.wait === 0 ? true : -750 + o.wait, ...elevationOf(o, 'both'), fadeIn: 0, fadeOut: 0 })); }
