...
// keyed by identity and resolved against the recipes — which FX answers, and what plays
...
import { sentence } from '../scripts/core/fx.js';
...
const ask = (it) => {
  ...
  return { name: it.name, type: it.type, keys: subject.keys, fx: r.fx?.id ?? null, key: r.key ?? null, source: r.source ?? null, sentence: r.fx ? sentence(r.fx, { name: it.name }) : 'Nothing plays yet.' };
};
...
for (const a of out.actors) {
  const plays = a.items.filter((i) => i.fx);
  const nothing = a.items.filter((i) => !i.fx);
  console.log(`\n${a.name} (${a.type}): ${plays.length} of ${a.items.length} play`);
  if (actorFilter) for (const i of a.items) console.log(`  ${i.fx ? '✓' : '·'} ${i.name} [${i.type}] · ${i.keys.join(', ')} → ${i.fx ? `${i.fx} (${i.source}, ${i.key})` : 'nothing'}`);
  else if (nothing.length) console.log(`  nothing yet: ${nothing.map((i) => i.name).join('; ')}`);
  const eff = a.effects.filter((e) => e.fx);
  if (a.effects.length) console.log(`  effects: ${eff.length} of ${a.effects.length} have an FX${actorFilter ? ` — ${a.effects.map((e) => `${e.name}${e.fx ? ` → ${e.fx}` : ''}`).join('; ')}` : ''}`);
}
for (const p of out.packs) { const plays = p.items.filter((i) => i.fx).length; console.log(`\n${p.pack}: ${plays} of ${p.items.length} play; nothing yet: ${p.items.filter((i) => !i.fx).map((i) => i.name).slice(0, 40).join('; ')}${p.items.length - plays > 40 ? ' …' : ''}`); }
console.log(`\n${out.answered} of ${out.asked} abilities have an FX`);
