...
try {
  ...
  const out = await f.evaluate(async ({ fx }) => {
    ...
    try {
      ...
      ok('§3 nothing was saved by the preview', api.fx.buffer().length === before.length, `${api.fx.buffer().length} in the buffer`);
      // 4 · save with provenance
      const saved = await api.fx.save(draft, { by: 'Tester Assistant' });
      ok('§4 saved to the world buffer with provenance', saved.ok && saved.fx.by === 'Tester Assistant' && /^\d{4}-\d{2}-\d{2}$/.test(saved.fx.at), JSON.stringify({ by: saved.fx?.by, at: saved.fx?.at }));
      ok('§4 the corpus resolves spell:sharran-step to it, from the world layer', api.resolve({ keys: ['spell:sharran-step'] }).fx?.id === 'sharran-step' && api.resolve({ keys: ['spell:sharran-step'] }).source === 'world', JSON.stringify(api.resolve({ keys: ['spell:sharran-step'] }).source));
      ...
      ok('§5 the card resolves it (a move, waiting for its click)', e?.fx === 'sharran-step' && e.source === 'world' && /destination/.test(e.why ?? ''), `${e?.fx} · ${e?.why}`);
    ...
    return { results, saved: api.fx.buffer().some((l) => l.id === 'sharran-step') };
  }, { fx: fixture });
  // 7 · the offline export reads the buffer from the world's database and lists the sentence
  if (out.saved) {
    let text = '';
    try { text = execFileSync(process.execPath, [join(REPO, 'tools', 'export-fx.mjs')], { encoding: 'utf8' }); } catch (e) { text = String(e.stdout ?? e.message); }
  ...
  const after = await f.evaluate(async () => { const api = game.modules.get('fvtt-mod-fxstudio').api; const r = await api.fx.remove('sharran-step'); return { removed: r.ok, left: api.fx.buffer().filter((l) => l.id === 'sharran-step').length, resolves: api.resolve({ keys: ['spell:sharran-step'] }).fx?.id ?? null }; }, null);
