// Does every FX in the corpus BUILD on the sandbox? Every FX is built by the engine against a
...
const { f, dispose } = await connectSandbox({ tag: 'fx', watchdogMs: 600_000 });
let fixture = null;
const since = Date.now();
try {
  fixture = await f.evaluate(fixtureUp, { items: [] });
  console.log(`[fx] fixture: scene ${fixture.sceneId}, caster ${fixture.casterTokenId}, target ${fixture.targetTokenId}`);
  const out = await f.evaluate(async ({ casterTokenId, targetTokenId, knownMissing }) => {
    ...
    const momentFor = (fx) => {
      const base = { subject: { name: fx.id, keys: fx.for }, source: caster, targets: [{ token: target, hit: true }], origin, id: 'fx' };
      if (fx.on === 'effect') return { ...base, when: 'effect', kind: 'effect', source: target, tie: null };
      if (needsPlace(fx)) return { ...base, when: 'use', kind: 'template', place: templates.circle, tie: templates.circle };
      if (fx.scenes.some((s) => s.shape === 'move')) return { ...base, when: 'use', kind: 'use', destination: { x: 850, y: 850 }, noMove: true };
      return { ...base, when: 'use', kind: 'use' };
    };
    for (const { fx, source } of list) {
      if (fx.off) continue;
      const shape = fx.scenes?.[0]?.shape ?? '?';
      results.byShape[shape] = (results.byShape[shape] ?? 0) + 1;
      try {
        const { seq, ctx } = api.build(fx, momentFor(fx));
        results.built++;
        if (!seq) results.empty.push(`${fx.id} (${shape})`);
        for (const m of ctx.missing) results.errors.push(`${fx.id} (${source}): ${m}`);
        for (const p of [...ctx.files.flat(), ...ctx.sounds]) { if (!p) { results.errors.push(`${fx.id} (${source}): a scene has no path`); continue; } (results.paths.get(p) ?? results.paths.set(p, []).get(p)).push(fx.id); }
      } catch (e) {
        results.errors.push(`${fx.id} (${source}): ${e.message}`);
      }
    }
    ...
    const partyOut = c.actors.map((a) => ({ name: a.name, plays: a.items.filter((i) => i.fx).length, items: a.items.length, effects: a.effects.filter((e) => e.fx).length, effectsAll: a.effects.length }));
    ...
    const npcOut = { attacks: attacks.length, plays: attacks.filter((i) => i.fx).length, byBase: attacks.filter((i) => i.fx && i.key && i.keys.indexOf(i.key) > 1).map((i) => `${i.name} → ${i.key}`).slice(0, 8), nothing: [...new Set(attacks.filter((i) => !i.fx).map((i) => i.name))].slice(0, 14) };

    await canvas.scene.deleteEmbeddedDocuments('Region', [circle.id, cone.id, square.id].filter((id) => canvas.scene.regions.get(id)));
    return { fx: results.fx, built: results.built, byShape: results.byShape, empty: results.empty, errors: results.errors, problems: results.problems, paths: results.paths.size, rawChecked: checked.size, missing: real, tolerated, party: partyOut, nothing: c.nothing.map((n) => `${n.actor}: ${n.name}`), npc: npcOut };
  }, { casterTokenId: fixture.casterTokenId, targetTokenId: fixture.targetTokenId, knownMissing: KNOWN_MISSING });
  console.log(`[fx] ${out.fx} fx · built ${out.built} · shapes ${Object.entries(out.byShape).map(([k, v]) => `${k} ${v}`).join(', ')}`);
  console.log(`[fx] ${out.paths} distinct paths named, ${out.rawChecked} raw files asked of the server`);
  ...
  if (out.empty.length) console.log(`  · ${out.empty.length} fx(s) built to nothing against this moment: ${out.empty.slice(0, 6).join('; ')}${out.empty.length > 6 ? '…' : ''}`);
  console.log(`[fx] the party: ${out.party.map((p) => `${p.name} ${p.plays}/${p.items} play, effects ${p.effects}/${p.effectsAll}`).join(' · ')}`);
  if (out.nothing.length) console.log(`  · nothing plays yet for ${out.nothing.length} abilities (the migration report lists them)`);
  console.log(`[fx] NPC attacks: ${out.npc.attacks}, ${out.npc.plays} play · by a later key, e.g. ${out.npc.byBase.join('; ') || '—'} · nothing: ${out.npc.nothing.join('; ') || '—'}`);
  const failed = out.errors.length + out.missing.length + out.problems.length;
  console.log(failed ? `FAIL: ${out.errors.length} build error(s), ${out.missing.length} missing path(s), ${out.problems.length} index problem(s)` : `PASS: ${out.built} fx build, every path resolves`);
  process.exitCode = failed ? 1 : 0;
} finally {
  if (fixture) { const d = await f.evaluate(fixtureDown, { ...fixture, since }).catch((e) => ({ error: e.message })); console.log(`[fx] teardown: ${JSON.stringify(d)}`); }
  await dispose();
}
