...
try {
  ...
  const out = await f.evaluate(async ({ fx }) => {
    ...
    try {
      ...
      await sleep(600);
      ok('§1 the window opens with its five tabs, Custom and Asset Library among them (Corpus waits behind the maintainer switch)', app?.rendered && $$('[role=tab]').length === 5 && /Custom/.test(text('.tabs')) && /Asset Library/.test(text('.tabs')), `${$$('[role=tab]').map((t) => t.textContent).join(', ')}`);
      ok('§1 Look up shows Misty Step as a sentence, and why', /Misty Step · when used/.test(text('.sentence')) && /has an FX for that/.test(text('.why')), `${text('.sentence')} | ${text('.why')}`);
      ok('§1 the card carries no sheet line, only the title and the sentence', !$('.result .owner'), text('.result .owner') || 'none');
      ok('§1 the card offers to create an FX from this one', /Create FX from this/.test(text('.actions')), text('.actions'));
      ...
      await click(hit);
      ok('§2 it plays nothing yet, and says so in words', /Nothing plays yet/.test(text('.sentence')) && /No FX answers the spell Sharran Step/.test(text('.why')), `${text('.sentence')} | ${text('.why')}`);
      ok('§2 the card offers to give it an FX, and a Close', /Give it an FX/.test(text('.actions')) && !!$('[data-act="close-card"]'), text('.actions'));
      ...
      ok('§3 Give it an FX opens the walk on step 2, step 1 already answered', $('[role=tab][aria-selected="true"]')?.dataset.tab === 'create' && stepNow() === 2 && $('.step[data-state="done"]')?.dataset.step === '1', `tab ${$('[role=tab][aria-selected="true"]')?.dataset.tab}, step ${stepNow()}`);
      ok('§3 step 2 offers duplicate, a starter and from scratch', $$('[data-act="cw-start"]').length === 3 && /Duplicate an existing fx/.test(text('.choices')) && /From scratch/.test(text('.choices')), text('.choices').slice(0, 120));
      ...
      ok('§4 what will be saved is read back on step 4, before Save', /dark black/.test(text('.walk .preview')) && /what will be saved/i.test(text('.walk .preview')) && !!$('[data-act="cw-save"]'), text('.walk .preview').slice(0, 200));
      ...
      ok('§4 the window lands on Look up with the custom FX', $('[role=tab][aria-selected="true"]')?.dataset.tab === 'lookup' && /custom/.test(text('.status')) && /dark black/.test(text('.sentence')) && /this world/.test(text('.why')), `${text('.status')} | ${text('.sentence')} | ${text('.why')}`);
      ...
      ok('§5 Custom lists Sharran Step first under For an ability, one line, not in the module yet, with Edit, Export and Delete', firstRow && /^Sharran Step/.test(firstRow.textContent.trim()) && /for any the spell Sharran Step/.test(firstRow.textContent) && /not in the module yet/.test(firstRow.textContent) && !/when used/.test(firstRow.textContent) && !!firstRow.querySelector('[data-act="export-fx"]') && !!firstRow.querySelector('[data-act="delete-fx"]'), firstRow?.textContent.replace(/\s+/g, ' ').slice(0, 200));
      await type('.fx-cq', 'sharran');
      await sleep(400);
      ok('§5 the search narrows the list', $$('.list .row').length === 1, `${$$('.list .row').length} rows`);
      ok('§5 two sub-tabs with counts: For an ability, Attached to an item', $$('[data-act="custom-kind"]').length === 2 && /For an ability · \d+/.test(text('.subtabs')) && /Attached to an item · \d+/.test(text('.subtabs')), text('.subtabs'));
      await click('[data-act="custom-kind"][data-kind="item"]');
      ok('§5 the item sub-tab carries the warning and its own rows', /may not work in another world/.test(text('[data-pane="custom"]')) && $$('.list .row').every((r) => !/for any/.test(r.textContent)), `${$$('.list .row').length} rows`);
      await click('[data-act="custom-kind"][data-kind="ability"]');

      // 6 · Corpus (behind the maintainer switch): what waits, binding and unbinding
      ok('§6 without the maintainer switch there is no Corpus tab', !$('[data-tab="corpus"]'), $$('[role=tab]').map((t) => t.textContent).join(', '));
      await game.settings.set(MOD, 'maintainer', true);
      await app.render(); await sleep(200);
      await click('[data-tab="corpus"]');
      ok('§6 Corpus lists the main corpus as rows, searchable, with a Maintain card', $$('.list .row').length >= 200 && /FX in the main corpus/.test(text('[data-pane="corpus"]')) && /Maintain · version/.test(text('[data-pane="corpus"]')), `${$$('.list .row').length} rows`);
      const row6 = $$('.list .row').find((r) => /Sharran Step/.test(r.textContent));
      ok('§6 Not yet shipped lists Sharran Step as a draft, with Bind for the house corpus', row6 && /draft/.test(row6.textContent) && !!row6.querySelector('[data-act="co-stage"][data-to="house"]'), row6?.textContent.replace(/\s+/g, ' ').slice(0, 160));
      ...
      await sleep(300);
      ok('§6 Bind for the house corpus binds it, and the Ship card names the version and the file', api.fx.buffer().find((l) => l.id === 'sharran-step')?.to === 'house' && /Write the corpus files and stamp/.test(text('[data-act="co-ship"]')) && /house\.json/.test(text('[data-pane="corpus"]')), text('[data-act="co-ship"]'));
      await click($$('.list .row').find((r) => /Sharran Step/.test(r.textContent))?.querySelector('[data-act="co-stage"][data-to=""]'));
      await sleep(300);
      ok('§6 Keep as a draft unbinds it', !api.fx.buffer().find((l) => l.id === 'sharran-step')?.to && !$('[data-act="co-ship"]'), '');
      await click($$('.list .row').find((r) => /Sharran Step/.test(r.textContent))?.querySelector('[data-act="co-stage"][data-to="house"]'));
      await sleep(300);
      ok('§6 bound again for the ship later', api.fx.buffer().find((l) => l.id === 'sharran-step')?.to === 'house', '');

      // 7 · one item's own FX, through the walk (from the card, duplicating the FX it has)
      api.open({ item: tmp });
      await sleep(300);
      await click('[data-act="create-from"]');
      ok('§7 Create FX from this opens the walk on step 3 with the FX copied', stepNow() === 3 && $$('.scene').length >= 2 && /A copy of Sharran Step/.test(text('.walk .lead')), `step ${stepNow()} · ${text('.walk .lead').slice(0, 40)}`);
      await click('[data-act="cw-next"]');
      ...
      await click(only);
      ok('§7 with "only this one" the read-back says it plays for that one item', /for that one Sharran Step/.test(text('.walk .preview')), text('.walk .preview').slice(-120));
      await click('[data-act="cw-save"]');
      await sleep(500);
      ...
      ok('§7 the item plays its own FX ahead of the spell\'s, and says so', r7.fx?.id === own?.id && /FX of its own/.test(r7.why), `${r7.sentence} | ${r7.why}`);
      ...
      ok('§7 the card offers to go back to the FX it had, and no Play nothing', /Back to the FX it had/.test(text('.actions')) && !$('[data-act="silence"]'), text('.actions'));
      ...
      await app.render();
      await sleep(300);
      ok('§9 Corpus lists the ship in Shipped from here and shows the new version', /the suite shipped Sharran Step/.test(text('[data-pane="corpus"]')) && text('[data-pane="corpus"]').includes(`version ${next.patch}`), text('[data-pane="corpus"]').match(/Maintain · version [^\s]+/)?.[0] ?? 'no version line');
      ...
      ok('§12 one compendium picked: the button says so', /Check 1 compendium/.test($('[data-act="books"]')?.textContent) && !$('[data-act="books"]').disabled, $('[data-act="books"]')?.textContent);
      await click('[data-act="books"]');
      for (let i = 0; i < 120 && !$('.tile'); i++) await sleep(250);
      const ctiles = $$('.tile').map((t) => t.textContent.replace(/\s+/g, ' ').trim());
      ok('§12 the book is read: abilities read, play an FX, play nothing yet', ctiles.length === 3 && /abilities read/.test(ctiles[0]) && /play an FX/.test(ctiles[1]), ctiles.join(' | '));
      ...
      ok('§13 the FX that use a style are named, with their variant', /In \d+ FX/.test(text('.lib-users')) && !!$('[data-act="lib-open-fx"]'), text('.lib-users').slice(0, 120));
      ...
      ok('§13 the Unused pill narrows the list, and the count above says so', unusedN < allSounds && text('.shelf .count').startsWith(`${unusedN} of ${allSounds}`), `${unusedN} of ${allSounds} · ${text('.shelf .count')}`);
      ...
      await click('[data-act="create-from"]');
      ok('§13 Create FX from this is on step 3 with a browse button on the first VFX', stepNow() === 3 && !!$('[data-act="cw-browse"][data-slot="asset"]'), `step ${stepNow()}`);
      await click('[data-act="cw-browse"][data-slot="asset"]');
      ok('§13 browse opens the Asset Library as the picker, saying what it picks for', $('[role=tab][aria-selected="true"]')?.dataset.tab === 'library' && /Picking the VFX for line 1 of Misty Step/.test(text('.picking')), text('.picking'));
      await click($$('[data-act="lib-sel"]').find((r) => r.dataset.id === 'jb2a.arcane_hand'));
      await click('[data-act="lib-pick-use"]');
      ok('§13 Use this VFX returns to step 3 with the line playing it', $('[role=tab][aria-selected="true"]')?.dataset.tab === 'create' && stepNow() === 3 && /arcane hand/i.test(text('.walk .preview')), text('.walk .preview').slice(0, 160));
      ...
      ok('§13 "what are these?" opens the legend', $$('.legend .row').length === 8, `${$$('.legend .row').length} rows`);
