// FILL: a picture sized to a placed template's shape (a circle, a cone, a line or a wall, a
// rectangle, a ring, an emanation around the caster — engine/places.js templateShape) — attached to
...
export function build(seq, scene0, ctx) {
  ...
  if (!region) return;
  const { type, width, height, directional } = templateShape(region);
  if (!type) return; // a shape this engine cannot size to: nothing plays, never a throw
  ...
  const defaultAnchor = directional ? { x: 0, y: 0.5 } : { x: 0.5, y: 0.5 };
  ...
  else e.size({ width: width * scale.x, height: height * scale.y });
