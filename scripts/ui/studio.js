...
// sheet, ui/sheet.js), Asset Library (ui/library.js), Audit (the compendiums the user picks, read
...
export class Studio extends ApplicationV2 {
  ...
  renderResult() {
    ...
    return `<h2>${esc(s.name)}<span class="status">${dot(status)}${STATUS_WORDS[status]}</span></h2>
      ...
      <div class="actions">
        <button type="button" class="primary" data-act="${r.fx ? 'open-sheet' : 'create-new'}">${r.fx ? 'Open FX' : 'Create FX'}</button>
  ...
  async onClick(ev) {
    ...
    if (['tab', 'entry', 'key', 'hit', 'new', 'create-new', 'open-sheet', 'edit-fx', 'open-fx'].includes(act) && sheetDirty(this) && !(await leaveSheet(this))) return undefined;
    ...
    switch (act) {
      ...
      case 'open-sheet': { const r = this.answer(); if (r?.original?.id) openSheet(this, { id: r.original.id, subject: S.subject }); return this.render(); }
      ...
      case 'edit-fx': openSheet(this, { id: b.dataset.id, edit: true }); return this.render();
